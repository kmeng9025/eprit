...
%     end
  ...
  cut_field_list = {'T1', 'tau', 'tau2'};
  for ii=1:length(cut_field_list)
    if isfield(FinalImage.raw_info, cut_field_list{ii})
      the_field = FinalImage.raw_info.(cut_field_list{ii});
      FinalImage.raw_info.(cut_field_list{ii}) = the_field(list_of_echoes);
    end
  end
...
if is_fit_data
  selection = 1:size(FinalImage.Raw,4);
  [FinalImage.fit_data] = epri_recovery_fit(FinalImage.Raw, FinalImage.raw_info.T1(selection)'*1E6, fields.fit);
  FinalImage.Dim4Type = 'Inversion';
