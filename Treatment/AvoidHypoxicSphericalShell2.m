function [isocenter,results] = AvoidHypoxicSphericalShell2(po2image,resolution,TumorMask,radius, results)
...
if exist('results', 'var')
  center = results.isocenter;
else
  [center, results] = TargetHypoxicSphere2(po2image,resolution,TumorMask,radius);
end
...
% 
% centerXYZ = center .* resolution;
% distmat = sqrt((X-centerXYZ(1)).^2 + (Y-centerXYZ(2)).^2 + (Z-centerXYZ(3)).^2);
