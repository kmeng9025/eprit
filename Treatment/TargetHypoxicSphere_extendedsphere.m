% Similar to TargetHypoxicSphere_extendedbox function but now finds perimeter
% of tumor mask and places a sphere at each point on this perimeter to
% extend tumormask

function [isocenter,results] = TargetHypoxicSphere_extendedsphere(po2image,radius,Mask)
...
%         tic
        ...
        %expand spheres from perimeter voxels:
        
        xxx = zeros(size(po2image));
        yyy = zeros(size(po2image));
        zzz = zeros(size(po2image));
        for iii = 1:size(po2image,1)
            for jjj = 1:size(po2image,2)
                for kkk = 1:size(po2image,3)
                    xxx(iii,jjj,kkk) = iii;
                    yyy(iii,jjj,kkk) = jjj;
                    zzz(iii,jjj,kkk) = kkk;
                end
        ...
        tumorperim = bwperim(tumormask);
        expanded = zeros(size(po2image));
        for ii = 1:size(po2image,1)
            for jj = 1:size(po2image,2)
                for kk = 1:size(po2image,3)
                    if tumorperim(ii,jj,kk) == 1
                        distmap = sqrt((xxx-ii).^2 + (yyy-jj).^2 + (zzz-kk).^2);
                        expanded(distmap <= radius) = 1;
                    end
                end
            end
        end
