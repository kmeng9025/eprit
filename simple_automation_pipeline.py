...
class SimpleEPRIAutomation:
    ...
    def load_fit_parameters(self, fit_data, param_name):
        ...
        if algorithm == 'T2_ExpDecay_No_Offset':
            ...
            # For T1 inversion recovery: typically [M0, T1, offset] or similar
            iAMP, iT1, iERR = 0, 1, 2  # M0 is amplitude
        else:
        ...
        if param_name.upper() == 'AMP':
            ...
            # For T1 data, we might not have T2, but return T1 if available
            if algorithm == 'T1_InvRecovery_3ParR1':
                fit_val[fit_data.Idx] = fit_data.P[iT1, :]  # Return T1 instead
            else:
            ...
            try:
                ...
                if i == 0 and 'fit_data' in p_mat:
                    try:
                        ...
                        print(f"   ⚠️  Error creating BE_AMP: {e}")
