        ...
        for(int kkz=-floorx(kernRes/2); kkz<=floorx(kernRes/2); kkz++){
            ...
            for(int kky=-floorx(kernRes/2); kky<=floorx(kernRes/2); kky++){
                ...
                for(int kkx=-floorx(kernRes/2); kkx<=floorx(kernRes/2); kkx++){
                    ...
                    int index = (kLocY-1) + (kLocX-1) * gridSize + (kLocZ-1) * gridSize*gridSize;
                    if (index < gridSize*gridSize*gridSize) {           
                        //pthread_mutex_lock(&mutex);
                        kspaceReal[index] += currKReal;
                        kspaceImag[index] += currKImag;
        ...
        )
{
    ...
    for(t=0; t<numWorker; t++) {
        ...
        args[t].kspaceReal = (double*)calloc(gridRes*gridRes*gridRes,sizeof(double));
        args[t].kspaceImag = (double*)calloc(gridRes*gridRes*gridRes,sizeof(double));
        ...
        /* wait some time before making the next thread */        
#ifdef WIN32
        sleep(1);
#endif
    ...
    for(t=0; t<numWorker; t++) {
        ...
        for(int i=0; i<gridRes*gridRes*gridRes; i++) {
            kspaceReal[i] += args[t].kspaceReal[i];
            kspaceImag[i] += args[t].kspaceImag[i];
        }
        if(args[t].kspaceReal)
            free(args[t].kspaceReal);
        if(args[t].kspaceImag)
            free(args[t].kspaceImag);        
    }
