function [bevmaps, depth_info, varargout] = maskbev_depths_func_general(CT, Target, material_mask  ,Bev_masks, angles, varargin)
...
Center_postion = 0;
if nargin > 6,  Center_postion = varargin{2}; end
 docoverage=0;
if nargin > 7, docoverage=varargin{3}; end
...
material_atten=[0.0206 0.021 0.029]; %attenuation 1/mm for materials when replacing acrylic ring with 3D printed bed.
...
if Center_postion == 1
itgt=round(size(bev_mask,1)/2);
jtgt=round(size(bev_mask,2)/2);
ktgt=round(size(bev_mask,3)/2);
else
...
imfig=figure;
midmask=squeeze(material_mask(:,:,ktgt));
...
if nargout > 2, varargout{1}=beamtimes; varargout{2}=[imfig,maskfig]; end
