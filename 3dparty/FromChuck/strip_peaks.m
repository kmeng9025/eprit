function [airmask, tismask, pvsmask] ...
    = strip_peaks(midslice, rsqmesh1, rin, varargin)

doimage=0;
doplot=0;
if nargin>3, if ~isempty(varargin{1}),
        imax=varargin{1}; doimage=1; end
end
if nargin>4, if ~isempty(varargin{2}),
        plotax=varargin{2}; doplot=1; end
end
...
thepeak=min(find(nuresid==max(nuresid(svals))));
...
if numel(goodpeak)>1,  % use the narrower one if more than one
