function [bevmaps, depth_info, varargout] = depth_dose_calculation(CT, material_mask,Target,Bev_masks,opts)
...
%Added another control to varargin. use a cell aray with the form {
%Prescription , Center_postion , docoverage }
%Center_postion = 0 is the normal maskbev_procedure, moving the treatement
%center to the center of the target. Center_postion = 1 leaves it at the
%center of the image.
%Matt Maggio, Dec 2016.


prescription = opts.prescription_Gy;
material_atten = safeget(opts, 'material_atten', [0.0206 0.021 0.029]); %attenuation 1/mm for materials when replacing acrylic ring with 3D printed bed.
calibration_atten = safeget(opts,'calibration_atten', 0.019);    % measured attenuation for solid water at 225 kVp
...
directions=[1 -1 -1];
pix_to_machine=hmatrix_translate(-Target([2,1,3]))...
    * hmatrix_scale(pixsiz*directions);

imfig=figure(1000); clf
subplot(1,2,1), imagesc(material_mask(:,:,Target(3))); axis image
colormap(gca,jet);
title(['material masks [', sprintf('%4.2f ',material_atten),']']);

subplot(1,2,2);
rx(:,1) = 1:jsize; rx(:,2) = 0; rx(3,3) = 0;
x=htransform_vectors(pix_to_machine,rx);
ry(:,2) = 1:isize; ry(:,1) = 0; ry(3,3) = 0;
y=htransform_vectors(pix_to_machine,ry);

imagesc(x(:,1), y(:,2), CT(:,:,Target(3))); axis image; colormap(gca, gray)
set(gca,'ydir','normal');
tgtxyz=htransform_vectors(pix_to_machine,Target([2,1,3]));
hold on
plot(tgtxyz(2),tgtxyz(1),'+');
title(sprintf('target (%i,%i,%i)',Target(2),Target(1),Target(3)))
...
%     bevmaps{end+1} = numap;
   ...
   % disp('Finding central axis depth for this beam');
    ...
    raymaskvals=interp3(xgrid,ygrid,zgrid,double(material_mask),xtrack,ytrack,ztrack,...
        'linear',0);
    raysum=0;
...
docoverage=0;
    ...
    material_mask=char(material_mask+coverage);
    toc;
    pause(0.1);
    end
...
if nargout > 2, varargout{1}=beamtimes; varargout{2}=imfig; end
