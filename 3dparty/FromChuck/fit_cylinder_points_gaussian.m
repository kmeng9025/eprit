...
plot(outer(:,1),outer(:,2),'+');
plot(inner(:,1),inner(:,2),'+');
[xin,yin,rin]=circle_from_points1(inner);
[xout,yout,rout]=circle_from_points1(outer);
%viscircles([xin yin;xout yout],[rin rout]);

% check to see if any points aren't close to the circles - sometimes get a
% couple of bad ones due to improper tabletop removal, or ring hitting the
% edge of the image
radvecs=inner-repmat([xin yin],size(inner,1),1);
rads=sqrt(sum(radvecs.*radvecs,2));
devs=abs(rads-rin);
goodones=find(devs<0.15*rin);
if numel(goodones)<size(inner,1),
    [xin,yin,rin]=circle_from_points1(inner(goodones,:));
end
radvecs=outer-repmat([xout yout],size(outer,1),1);
rads=sqrt(sum(radvecs.*radvecs,2));
devs=abs(rads-rout);
goodones=find(devs<0.15*rout);
if numel(goodones) < size(outer,1),
    [xout,yout,rout]=circle_from_points1(outer(goodones,:));
end
...
subplot(1,4,3);
hold off;
[airmask,skinmask,pvmask]=strip_peaks(midslice,rsqmesh1,rin,[],gca);

legvol(:,:,midplane)=skinmask;
ringvol(:,:,midplane)=ringmask;
pvsvol(:,:,midplane)=pvmask;
totmask=ringmask+2*skinmask+3*pvmask;
figure (plotfig); subplot(1,4,4);
imagesc(totmask),axis image 
